...
get '/memo/:memo_id/editor' do # メモの編集ページ
  memo_data = DATABASE.exec_params('SELECT * FROM memo WHERE id = $1', [params[:memo_id]]).values[0]
  redirect(not_found) if memo_data.nil?
  @memo = { memo_id: memo_data[0], memo_name: memo_data[1], memo_body: memo_data[2] }
  erb :editor_memo
end
