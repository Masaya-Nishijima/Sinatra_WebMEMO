...
get '/memo/:memo_id/editor' do # メモの編集ページ
  @memo = get_memo_data(params[:memo_id])
  erb :editor_memo
end
...
def get_memo_data(memo_id)
  memo_data = DATABASE.exec_params('SELECT * FROM memo WHERE id = $1', [memo_id]).values[0]
  redirect(not_found) if memo_data.nil?
  { memo_id: memo_data[0], memo_name: memo_data[1], memo_body: memo_data[2] }
end
